...
def propagate_pooling(relevant, irrelevant, pooler):
    ...
    avg_pooler = torch.nn.AvgPool2d(
        kernel_size=(pooler.kernel_size, pooler.kernel_size),
...
def cd(blob, im_torch, model, device="cuda"):
    """
    Computes and returns decomposition of logits in to relevant and irrelevant

    :param blob: is the mask of same size as the input that identifies relevant through blob*im_torch
    :param im_torch: input
    :param model: model such that model(im_torch) is decomposed
    :param device: compute device

    :returns: relevant and irrelevant tensors of size [batch_size x num_classes] each
    """

    # set up model
    # original code turns on eval without setting it to train again, I do not see why model needs to be in eval
    # model.eval()
    ...
    # original code had special handling for mnist for some reason
...
def cd_batch_text(batch, model, start, stop, my_device=0):
    ...
    for i in range(T):
        ...
        if i > 0:
            ...
            irrelevant = (
                irrelevant
                + (rel_contrib_f + irrel_contrib_f + bias_contrib_f) * prev_irrel
                + irrel_contrib_f * prev_rel
            )
...
def cd_text(batch, model, start, stop, batch_id=0, my_device=0):
    ...
    tolerance = 0.001
    assert (
        torch.sum(torch.abs((model.forward(batch) - model.hidden_to_label.bias.data) - (scores + irrel_scores)))
        .cpu()
        .detach()
        .numpy()
        < tolerance
...
def cd_penalty_annotated(batch, model1, start, stop, scores):
    ...
    model_output = cd_text_irreg_scores(batch.text[:, idx_nonzero], model1, start[idx_nonzero], stop[idx_nonzero])[
        0
    ]  # get the output and focus on relevant scores for class 0 vs 1
    model_softmax = torch.nn.functional.softmax(model_output, dim=0)[
        batch.label[idx_nonzero], np.arange(len(idx_nonzero))
    ]  # take softmax of class 0 vs 1 and take the correct digit
    output = -(
        torch.log(model_softmax) * scores[idx_nonzero].float()
    ).mean()  # -(torch.log(1-model_softmax)*(1- scores[ idx_nonzero]).float() ).mean() #if it agrees, maximize - if it dis, min
    return output
...
def cd_penalty_for_one_decoy_all(batch, model1, start, stop):
    ...
    if mask_exists.any():
        ...
        model1_softmax = softmax_out(
            (model1_output[0][correct_idx], model1_output[1][correct_idx])
        )  # + softmax_out((model1_output[0][wrong_idx],model1_output[1][wrong_idx]))
...
def cd_penalty(batch, model1, model2, start, stop):
    ...
    return (
        ((model1_softmax - model2_softmax) * (torch.log(model1_softmax) - torch.log(model2_softmax)))
        .sum(dim=1)
        .reshape((2, -1))
        .sum(dim=0)
    )
